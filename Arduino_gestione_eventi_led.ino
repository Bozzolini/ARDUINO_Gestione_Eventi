...
const int BTN_SET = 2, BTN_PIU = 3, BTN_MENO = 4, BTN_OK = 5, BTN_EMER = 7, PIN_LED = 6;
...
void setup() {
  ...
  pinMode(BTN_EMER, INPUT_PULLUP); pinMode(PIN_LED, OUTPUT);
...
// Gestisce il lampeggio del LED per gli eventi in agenda senza bloccare il sistema
void gestisciLed() {
  static unsigned long inizioAllarme = 0;
  static unsigned long ultimoLampeggio = 0;
  static bool ledStato = false;
  ...
  if (allarmeAttivo) {
    if (millis() - inizioAllarme >= 5000) {
      ...
      // Logica del lampeggio (500ms)
      if (millis() - ultimoLampeggio > 500) {
        ultimoLampeggio = millis();
        ledStato = !ledStato;
        digitalWrite(PIN_LED, ledStato);
      }
    }
  } else {
...
// Esegue il ciclo di allarme di emergenza (5 secondi di lampeggio)
void emergenza() {
  allarmeAttivo = false; 
  for(int i = 0; i < 13; i++) { 
